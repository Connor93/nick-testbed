...
/**
 * Created by connorfraser on 11/19/2013.
 */
public class CustomConfigChooser extends ConfigChooser {
    // ===========================================================
    // Constants
    // ===========================================================
    ...
    private static final int[] EGLCONFIG_ATTRIBUTES_MULTISAMPLE = {
            ...
            EGL10.EGL_RENDERABLE_TYPE, EGL_GLES2_BIT,
            EGL10.EGL_SAMPLE_BUFFERS, 1,
            EGL10.EGL_SAMPLES, MULTISAMPLE_COUNT,
            EGL10.EGL_NONE
    };
    ...
    private static final int[] EGLCONFIG_ATTRIBUTES_COVERAGEMULTISAMPLE_NVIDIA = new int[]{
            ...
            EGL_COVERAGE_SAMPLES_NV, MULTISAMPLE_COUNT,  // always 5 in practice on tegra 2
            EGL10.EGL_NONE
    };
    ...
    public CustomConfigChooser(final boolean pMultiSamplingRequested) {
        super(pMultiSamplingRequested);
    ...
    private static int getEGLConfigCount(final EGL10 pEGL, final EGLDisplay pEGLDisplay, final int[] pEGLConfigAttributes) {
        if(pEGL.eglChooseConfig(pEGLDisplay, pEGLConfigAttributes, null, 0, BUFFER) == false) {
    ...
    private EGLConfig findEGLConfig(final EGL10 pEGL, final EGLDisplay pEGLDisplay, final EGLConfig[] pEGLConfigs, final ConfigChooserMatcher pConfigChooserMatcher) {
        for(int i = 0; i < pEGLConfigs.length; ++i) {
            ...
            if(config != null) {
                final int redSize = getConfigAttrib(pEGL, pEGLDisplay, config, EGL10.EGL_RED_SIZE, 0);
                final int greenSize = getConfigAttrib(pEGL, pEGLDisplay, config, EGL10.EGL_GREEN_SIZE, 0);
                final int blueSize = getConfigAttrib(pEGL, pEGLDisplay, config, EGL10.EGL_BLUE_SIZE, 0);
                final int alphaSize = getConfigAttrib(pEGL, pEGLDisplay, config, EGL10.EGL_ALPHA_SIZE, 0);
                final int depthSize = getConfigAttrib(pEGL, pEGLDisplay, config, EGL10.EGL_DEPTH_SIZE, 0);
                final int stencilSize = getConfigAttrib(pEGL, pEGLDisplay, config, EGL10.EGL_STENCIL_SIZE, 0);
    ...
    public enum ConfigChooserMatcher {
        ...
        STRICT() {
            ...
            public boolean matches(final int pRedSize, final int pGreenSize, final int pBlueSize, final int pAlphaSize, final int pDepthSize, final int pStencilSize) {
                if(pDepthSize == DEPTH_SIZE && pStencilSize == STENCIL_SIZE) {
                    if(pRedSize == RED_SIZE && pGreenSize == GREEN_SIZE && pBlueSize == BLUE_SIZE && pAlphaSize == ALPHA_SIZE) {
                        return true;
                    }
                }
                return false;
